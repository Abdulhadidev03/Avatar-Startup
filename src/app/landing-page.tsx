...
function Icon({ name, size = 18 }: { name: IconName; size?: number }) {
  const paths: Record<IconName, React.ReactNode> = {
    ...
    code: (
      ...
      <path d="M6.3 15.8a8.5 8.5 0 0 1 11.4 0l1.2-2.2a1.5 1.5 0 0 0-.45-1.95 10.75 10.75 0 0 0-12.9 0 1.5 1.5 0 0 0-.45 1.95l1.2 2.2Z" />
    ),
    eye: (
      <>
    ...
    spark: (
      <>
        <path d="m12 2 1.2 4.1a6.5 6.5 0 0 0 4.5 4.5L22 12l-4.3 1.4a6.5 6.5 0 0 0-4.5 4.5L12 22l-1.2-4.1a6.5 6.5 0 0 0-4.5-4.5L2 12l4.3-1.4a6.5 6.5 0 0 0 4.5-4.5Z" />
      </>
    ),
    volume: (
      <>
...
function useDockedRect(
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const compactWidth =
        viewportWidth < 760 ? Math.min(viewportWidth - 24, 390) : 352;
      ...
      const targetWidth =
        expanded && progress > 0.7 ? panelWidth : compactWidth;
      const targetHeight =
        expanded && progress > 0.7 ? panelHeight : compactHeight;
  ...
  try {
    const candidates = Array.from(
      document.querySelectorAll<HTMLElement>(
        "h1,h2,h3,[data-section],section[aria-label]",
      ),
    );
    ...
    const label = (chosen.innerText || chosen.getAttribute("aria-label") || "")
      .trim()
      .replace(/\s+/g, " ")
      .slice(0, 80);
    return label || null;
...
function useVisitorContext() {
  ...
  useEffect(() => {
    ...
    const remember = (event: JourneyEvent) => {
      recentEventsRef.current = [...recentEventsRef.current, event].slice(
        -JOURNEY_LIMIT,
      );
      try {
        window.sessionStorage.setItem(
          JOURNEY_KEY,
          JSON.stringify(recentEventsRef.current),
        );
      } catch {
    ...
    try {
      ...
      if (Array.isArray(stored))
        recentEventsRef.current = stored.slice(-JOURNEY_LIMIT);
    } catch {
...
const steps = [
  {
    number: "01",
    title: "Choose a face",
    body: "Select a Ruhana avatar or create one from a photo you own.",
  },
  {
    number: "02",
    title: "Give it context",
    body: "Add your website, files, voice, tone, and the outcomes that matter.",
  },
  {
    number: "03",
    title: "Shape the conversation",
    body: "Set goals, guardrails, handoffs, and actions in plain language.",
  },
  {
    number: "04",
    title: "Copy one line",
    body: "Preview it, publish it, and follow every useful result from the dashboard.",
  },
];
...
const builderAvatars = [
  { name: "Sarah", kind: "Real", imageUrl: avatarById("sarah").imageUrl },
  {
    name: "Sarah",
    kind: "Animated",
    imageUrl: "/avatars/sarah-animated-v1.webp",
  },
...
const roles = [
  {
    name: avatarById("sarah").name,
    role: avatarById("sarah").title,
    imageUrl: avatarById("sarah").imageUrl,
    line: "Turns comparison into clarity.",
  },
  {
    name: avatarById("anne").name,
    role: avatarById("anne").title,
    imageUrl: avatarById("anne").imageUrl,
    line: "Resolves the question in context.",
  },
  {
    name: avatarById("gabriel").name,
    role: avatarById("gabriel").title,
    imageUrl: avatarById("gabriel").imageUrl,
    line: "Shows the next useful step.",
  },
];
...
export default function LandingPage({ authenticated }: LandingPageProps) {
  ...
  useEffect(() => {
    const stepNodes = Array.from(
      document.querySelectorAll<HTMLElement>("[data-builder-step]"),
    );
    ...
    <div className="landing-root">
      ...
      <header className="lp-nav" data-raised={navRaised}>
        ...
        <div className="lp-nav-actions">
          {!authenticated && (
            <Link className="lp-text-link" href="/sign-in">
              Sign in
            </Link>
          )}
          <Link
            className="lp-button lp-button-dark lp-button-small"
            href={primaryHref}
          >
            {primaryLabel}
      ...
      <main id="main-content">
        <section className="lp-hero" id="product">
          <div className="lp-hero-copy" data-reveal>
            <span className="lp-eyebrow">
              <i /> Context-aware video agents
            </span>
            <h1>
              An agent that understands the page and{" "}
              <em>knows what to do next.</em>
            </h1>
            <p>
              Ruhana sees the products, clicks, questions, and intent behind
              every visit. Then it talks, guides, and takes action.
            </p>
            <div className="lp-hero-actions">
              <Link className="lp-button lp-button-dark" href={primaryHref}>
                {primaryLabel}
                <Icon name="arrow" />
              </Link>
              <a className="lp-button lp-button-light" href="#context">
                See how it works
                <Icon name="chevron" />
              </a>
            </div>
            <div className="lp-hero-facts" aria-label="Product highlights">
              <span>
                <Icon name="check" size={15} /> Four guided steps
              </span>
              <span>
                <Icon name="check" size={15} /> One-line install
              </span>
              <span>
                <Icon name="check" size={15} /> Voice and chat
              </span>
            </div>
          </div>
          <div className="lp-hero-stage" ref={heroDemoRef} aria-hidden="true">
            <div className="lp-stage-loading">
              Preparing the live Ruhana preview…
            </div>
          </div>
          <div className="lp-scroll-cue">
            <span>Scroll to watch it become your widget</span>
            <i />
          </div>
        </section>
        ...
        <section className="lp-context-section lp-section" id="context">
          <div className="lp-section-heading" data-reveal>
            ...
            <h2>
              It reads the room.
              <br />
              <em>Because it reads the page.</em>
            </h2>
            <p>
              A generic chatbot waits for the visitor to explain everything.
              Ruhana already understands the page, the journey, and the signals
              around the question.
            </p>
          </div>
          <div className="context-browser" data-reveal>
            <div className="browser-bar">
              ...
              <i />
              <i />
              <span>yourstore.com/products/studio-chair</span>
              <b>Live context</b>
            </div>
            <div className="browser-content">
              <div className="browser-product-art">
                <div className="chair-art">
              ...
              <div className="browser-product-copy">
                ...
                <h3>Studio chair</h3>
                <p>
                  Designed for long focus. Built from recycled aluminium and
                  natural wool.
                </p>
            ...
            <div className="context-signal signal-one">
              <span>01</span>
              <div>
                <small>Page</small>
                <strong>Studio chair</strong>
              </div>
            </div>
            <div className="context-signal signal-two">
              <span>02</span>
              <div>
                <small>Behaviour</small>
                <strong>Compared 3 finishes</strong>
              </div>
            </div>
            <div className="context-signal signal-three">
              <span>03</span>
              <div>
                <small>Intent</small>
                <strong>Delivery concern</strong>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="lp-action-section lp-section">
          <div className="action-grid">
            <div className="action-copy" data-reveal>
              ...
              <h2>
                From hesitation to a <em>useful next step.</em>
              </h2>
              <p>
                Ruhana does more than answer. It can recommend, compare, capture
                details, book a meeting, begin checkout, or hand the
            ...
            <div className="action-sequence" data-reveal>
              <div className="action-question">
                <span>Visitor</span>
                <p>Will this fit under a 72 cm desk?</p>
              </div>
              <div className="action-route">
                <i />
                <span>Ruhana considers page + product + intent</span>
                <i />
              </div>
              <div className="action-answer">
                <span>Ruhana</span>
                <p>
                  Yes. With the standard base, the arms lower to 68 cm. I can
                  set that configuration for you.
                </p>
              </div>
              <div className="action-outcome">
                ...
                <div>
                  ...
                  <strong>Configure standard base</strong>
                </div>
        ...
        <section className="lp-builder-section lp-section" id="how-it-works">
          <div className="builder-intro" data-reveal>
            ...
            <h2>
              From idea to live
              <br />
              <em>in four clear steps.</em>
            </h2>
            <p>
              No prompt engineering. No maze of settings. Every screen answers
              one question and moves you closer to a working agent.
            </p>
          </div>
          <div className="builder-layout">
            <div className="builder-preview" data-step={activeStep}>
              <div className="builder-window">
                <div className="builder-window-head">
                  <RuhanaLogo href="/" compact />
                  <span>New agent</span>
                  <b>Step {activeStep + 1} of 4</b>
                </div>
        ...
        <section className="lp-roles-section lp-section" id="use-cases">
          <div className="roles-heading" data-reveal>
            <div>
              <span className="lp-index">04 · One platform, many roles</span>
              <h2>
                The right presence
                <br />
            ...
            <p>
              Start with a Ruhana avatar or bring your own photo. Give every
              role its own knowledge, voice, goals, and handoff rules.
            </p>
          </div>
          ...
          <p className="asset-note">
            Ruhana original avatar collection · Your custom avatars remain
        ...
        <section className="lp-impact-section lp-section" id="impact">
          <div className="impact-heading" data-reveal>
            ...
            <h2>
              See what changed
              <br />
              <em>after the conversation.</em>
            </h2>
            <p>
              Not a vanity dashboard. Ruhana connects conversations to outcomes
              so you can see where it helped—and where the experience still
              needs work.
            </p>
          </div>
          <div className="impact-dashboard" data-reveal>
            <div className="impact-topbar">
              ...
              <button type="button">
                Last 30 days <Icon name="chevron" size={14} />
              </button>
            </div>
            <div className="impact-metrics">
              ...
              <article>
                <span>Qualified visitors</span>
                <strong>184</strong>
                <small>From 612 conversations</small>
              </article>
              <article>
                <span>Resolved in session</span>
                <strong>76%</strong>
                <small>Without a handoff</small>
              </article>
            </div>
            <div className="impact-main">
              <div className="impact-chart-card">
                <div className="impact-card-head">
                  <div>
                    ...
                    <span>Conversation → useful result</span>
                  </div>
                ...
                <div className="impact-chart">
                  {[34, 42, 37, 51, 48, 58, 53, 66, 62, 71, 68, 78].map(
                    (height, index) => (
              ...
              <div className="impact-results-card">
                <div className="impact-card-head">
                  <div>
                    <strong>What Ruhana did</strong>
                    <span>Completed actions</span>
                  </div>
                </div>
                {[
                  ["Product recommendation", "82", 82],
                  ["Lead qualified", "47", 62],
                  ["Meeting booked", "31", 44],
                  ["Human handoff", "24", 34],
          ...
          <div className="deploy-card" data-reveal>
            <div className="deploy-copy">
              ...
              <h2>
                One line between
                <br />
                built and <em>live.</em>
              </h2>
              <p>
                Paste the Ruhana snippet before your closing body tag. The
                widget inherits its saved identity, behaviour, and knowledge
                automatically.
              </p>
              <div className="deploy-list">
                <span>
                  <Icon name="check" /> Works on any website
                </span>
                <span>
                  <Icon name="check" /> Responsive by default
                </span>
                <span>
                  <Icon name="check" /> Versioned and reversible
                </span>
              </div>
            </div>
            <div className="code-window">
              <div className="code-head">
                <span>
                  <Icon name="code" size={16} /> Install widget
                </span>
                <b>HTML</b>
              </div>
              <pre aria-label="Example widget installation code">
                <code>
                  <span>&lt;script</span>
                  {"\n  "}
                  <b>src</b>=
                  <em>&quot;https://cdn.ruhana.ai/widget.js&quot;</em>
                  {"\n  "}
              ...
              <div className="code-success">
                ...
                <div>
                  ...
                  <small>Visible on all published pages</small>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="lp-final-section">
          <div className="final-avatar-field" aria-hidden="true">
            {[0, 1, 2, 4, 5, 6, 7].map((index) => (
              <AtlasAvatar index={index} key={index} />
            ))}
          </div>
          <div className="final-copy" data-reveal>
            <span className="lp-eyebrow">
              <i /> Ready when your visitors are
            </span>
            <h2>
              Make every visit
              <br />
              <em>feel understood.</em>
            </h2>
      ...
      <footer className="lp-footer">
        <div className="footer-top">
          <RuhanaLogo href="/" />
          <p>
            Context-aware video agents for the moments that move a business
            forward.
          </p>
        </div>
        <div className="footer-links">
          <div>
            ...
            <a href="#impact">Analytics</a>
            <Link href="/pricing">Pricing</Link>
          </div>
          <div>
            <span>Build</span>
            <Link href={primaryHref}>Create an agent</Link>
        ...
        <div className="footer-bottom">
          <span>© {new Date().getFullYear()} Ruhana AI</span>
          <span>Built for useful conversations.</span>
        </div>
      </footer>
    </div>
  );
